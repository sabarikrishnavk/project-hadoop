...
public class MusicScoreJob extends Configured implements Tool {
...
//		args = new String[] { "input/input.txt", "output/test/"};
	...
	public static void main(String[] args) throws Exception {
		...
		int returnStatus = ToolRunner.run(new Configuration(), new MusicScoreJob(),
			args);
	...
	public int run(String[] args) throws IOException {
		...
		Job job =null;
		try {
			job =  Job.getInstance(conf, "Music score job");
			job.setJarByClass(MusicScoreJob.class);

			job.setMapperClass(SongScoreMapper.class);
				job.setMapOutputKeyClass(Text.class);
				job.setMapOutputValueClass(DoubleWritable.class);
			job.setCombinerClass(SongScoreReducer.class);
			job.setReducerClass(SongScoreReducer.class);
